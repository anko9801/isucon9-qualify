...
func getUserID(r *http.Request) (user User, errCode int, errMsg string) {
	session := getSession(r)
	userID, ok := session.Values["user_id"]
	if !ok {
		return user, http.StatusNotFound, "no session"
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	categoryIDs, ok := childCategories[rootCategory.ID]
	if !ok {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		return
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		apis.Store("pstr", pstr)
	}()
	wg.Wait()

	_scr, ok := apis.Load("scr")
	scr := _scr.(*APIShipmentCreateRes)
	if !ok {
		log.Print(err)
	...
	_pstr, ok := apis.Load("pstr")
	pstr := _pstr.(*APIPaymentServiceTokenRes)
	if !ok {
		log.Print(err)
...
func postLogin(w http.ResponseWriter, r *http.Request) {
	...
	for i := range u.HashedPassword {
		if u.HashedPassword[i] != hash[i] {
			outputErrorMsg(w, http.StatusUnauthorized, "アカウント名かパスワードが間違えています")
			return
		}
	}
